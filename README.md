...
9. **Blocking quality gate** (`src/lib/ai/quality-review.ts`) — an AI creative-director audit scored 0–100 across ten taste dimensions combined with deterministic scans (generic-phrase blocklist, banned claims, feature-section count, SEO shape) plus hard caps: a weak hero or fewer than 2 business-specific sections can never score ≥90; four-plus generic phrases or any banned claim forces the score below 85. The gate is **blocking, not advisory**: a draft under **90 is a failed generation**. The audit's issues are fed back into the next attempt (copy rewrite; below **85** the entire creative direction is regenerated) for up to **5 attempts**. If none passes, the best attempt is saved as `failed_quality_gate` — shown as a diagnostic draft with the remaining issues, never as a completed website. The gate applies to every mode, including "rewrite copy" / "change style" / layout rerolls. A structural-signature uniqueness check (layout + hero variant + palette family + section mix) flips the hero treatment when two businesses would receive the same site.
