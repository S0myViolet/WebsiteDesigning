...
3a. **Reference benchmarks** — a Settings-managed library of design-quality reference sites per category (7 premium Dubai hospitality sites ship as defaults). Their extracted patterns (reservation-first navigation, proud menu categories, atmosphere sections, location blocks) set the bar the generated site must reach; the auditor applies an "embarrassment test" against them. Patterns only — names, text, layouts, and assets are never copied.
3b. **Visual grounding** (`src/lib/ai/visual-cues.ts`) — up to 3 of the business's public Google photos are fetched via the official Places Photo API and analyzed by an AI vision pass into design cues: the venue's dominant/accent colors, lighting mood, materials, day/night feel, and vibe. The palette is derived from these cues (never random); photos are analyzed in-memory only and never embedded in generated sites. Restaurants/cafes additionally get a hospitality direction layer (cuisine identity, visit moment, menu presentation, CTA strategy, required modules).
...
9. **Blocking quality gate** (`src/lib/ai/quality-review.ts`) — an AI creative-director audit scored 0–100 across ten taste dimensions combined with deterministic scans (generic-phrase blocklist, banned claims, feature-section count, SEO shape) plus hard caps: a weak hero or fewer than 2 business-specific sections can never score ≥90; four-plus generic phrases or any banned claim forces the score below 85. The gate is **blocking, not advisory**: a draft under **90 is a failed generation**, and a 90–92 draft only stops the loop when no generic phrasing or high-severity issues remain (target **93+**). Failed audits feed a deliberate **escalation ladder** across up to **8 attempts**: surgical copy revision → new creative direction + layout → palette re-derived from the venue's visual cues → stronger business-specific modules → hero redesign → a different layout family → a final aggressive pass built from every prior failure. Hospitality businesses must carry at least **3** business-specific modules (others 2). If nothing passes, the best attempt is saved as `failed_quality_gate` — a diagnostic draft, never a completed website. The gate applies to every mode, including "rewrite copy" / "change style" / layout rerolls (copy-only rerolls cap at 4 attempts). A structural-signature uniqueness check (layout + hero variant + palette family + section mix) flips the hero treatment when two businesses would receive the same site.
10. **Save** — copy, creative direction, design system, brief, style, layout, quality report, gate status/attempts, uniqueness notes, preview HTML, and export code are all persisted.
...
| POST | `/api/businesses/[id]/generate-website` | `{mode?: "full"\|"copy"\|"style"}` | `{website: WebsiteDto, research: ResearchResult, generation: {status, score, attempts, maxAttempts, message}}` — runs the quality-gated pipeline (up to 5 attempts; below 90 = `failed_quality_gate`). `full` (default) runs everything, `copy` rewrites copy only, `style` regenerates brief/style/layout keeping the copy. Auto-runs the analysis if missing |
| GET | `/api/businesses/[id]/generation-progress` | — | `{active: false}` or `{active: true, stage, attempt, maxAttempts, bestScore}` — polled by the UI while a generation runs |
